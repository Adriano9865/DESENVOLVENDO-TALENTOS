  ...
  template: `
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fade-in" (click)="closeModal()">
      <div class="bg-white rounded-lg shadow-xl p-8 w-full max-w-md relative animate-fade-in-up" (click)="$event.stopPropagation()">
        ...
        <h2 class="text-2xl font-bold text-slate-800 mb-6 text-center">Login do Aluno</h2>
        ...
        <form (submit)="onLogin(); $event.preventDefault()">
          ...
          <div class="mt-6">
            <button type="submit" class="w-full bg-indigo-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-transform transform hover:scale-105 active:scale-100 disabled:bg-indigo-400 disabled:cursor-not-allowed" [disabled]="login().length !== 4 || password().length !== 4">
              Entrar
            </button>
          </div>
        </form>
...
export class StudentLoginModalComponent {
  ...
  onLogin() {
    ...
    const student = this.studentService.validateLogin(this.login(), this.password());
    if (student) {
      this.loginSuccess.emit(student);
    } else {
